...
import { Shield, Search, Plus, Edit, Eye, Trash2, RefreshCw, Loader2, X, Check, CheckCircle2, XCircle } from "lucide-react";
...
export default function RolePage() {
  ...
  const { data: rolesData, isLoading, refetch, isFetching } = useQuery({
    queryKey: ["roles"],
    queryFn: fetchRoles,
  });
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => setNotification({ type: "error", message: err.response?.data?.error || "Failed to create Role" })
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["roles"] });
      setModalOpen(false);
      setNotification({ type: "success", message: "Role data updated successfully!" });
    },
    onError: (err: any) => setNotification({ type: "error", message: err.response?.data?.error || "Failed to update Role" })
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => {
      setDeleteModalOpen(false);
      setNotification({ type: "error", message: err.response?.data?.error || "Failed to delete Role" });
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (modalType === "create") {
      ...
      updateMutation.mutate({ id: selectedRoleId!, data: formData });
    }
  };
      ...
      <div className="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-semibold tracking-tight text-foreground">Roles List</h1>
          </div>
          <p className="text-[14px] font-medium text-muted-foreground">Manage access levels, system permissions, and user rights categories.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <Plus className="w-4.5 h-4.5" /> Create Role
          </button>
        </div>
      </div>
      ...
      <div className="enterprise-card flex flex-col overflow-hidden">
        ...
        <div className="p-5 border-b border-border flex items-center justify-between bg-muted/20">
          <div className="relative w-full max-w-sm">
              ...
              <input
                type="text"
                placeholder="Search feature or data..."
                value={searchTerm}
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="bg-muted/30 border-b border-border">
                ...
                <th className="px-6 py-4 font-bold text-muted-foreground text-[11px] uppercase tracking-wider w-48">Role ID</th>
                <th className="px-6 py-4 font-bold text-muted-foreground text-[11px] uppercase tracking-wider">Role Name</th>
                <th className="px-6 py-4 font-bold text-muted-foreground text-[11px] uppercase tracking-wider text-right">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              ...
              {!isLoading && filteredRoles.map((r: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4.5 font-bold font-mono text-[12px] text-foreground">{r.id}</td>
                    <td className="px-6 py-4.5 font-bold text-foreground">{r.name}</td>
                    <td className="px-6 py-4.5 text-right font-bold text-muted-foreground/60 text-[11px]">REGISTERED</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="p-5 border-t border-border flex flex-wrap items-center justify-between bg-muted/10">
          ...
          <div className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">
            {selectedRoleId ? `Selected Role: ${selectedRole?.name}` : 'Select a role to modify data'}
          </div>
        </div>
        ...
        <Pagination 
          currentPage={1}
          totalPages={Math.max(1, Math.ceil(roles.length / 10))}
          onPageChange={() => {}}
          ...
          <div className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-border flex items-center justify-between bg-card">
              <h2 className="text-xl font-bold text-foreground">
                {modalType === "create" ? "Add New Role" : modalType === "view" ? "Role Details" : "Edit Role"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-muted/10">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Role Name</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Enter role permissions description..."
              ...
              <div className="pt-4 flex items-center gap-3">
                ...
                {modalType !== "view" && (
                  <button
                    ...
                    className="flex-1 h-12 rounded-xl bg-primary text-primary-foreground text-[14px] font-bold hover:bg-primary/90 transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/25"
                  >
                    {(createMutation.isPending || updateMutation.isPending) ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                    {modalType === "create" ? "Add Role" : "Save Changes"}
                  </button>
                )}
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setDeleteModalOpen(false)}>
          <div className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="p-8 text-center space-y-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-foreground">Delete Role?</h3>
                <p className="text-[14px] text-muted-foreground mt-1 px-4 leading-relaxed">
                  Are you sure you want to delete <span className="font-bold text-foreground">"{selectedRole.name}"</span>? Systems relying on this role might be affected.
                </p>
              </div>
              <div className="flex flex-col gap-2 pt-2">
                <button
                  onClick={() => deleteMutation.mutate(String(selectedRoleId!))}
                ...
                >
                  {deleteMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Yes, Delete Role"}
                </button>
                <button
