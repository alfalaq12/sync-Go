...
export default function RolePage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setModalOpen(false);
      resetForm();
      setNotification({ type: "success", message: "Hak Akses berhasil dibuat!" });
    },
    onError: (err: any) => setNotification({ type: "error", message: err.response?.data?.error || "Gagal membuat Hak Akses" })
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["roles"] });
      setModalOpen(false);
      setNotification({ type: "success", message: "Data Hak Akses berhasil diperbarui!" });
    },
    onError: (err: any) => setNotification({ type: "error", message: err.response?.data?.error || "Gagal memperbarui Hak Akses" })
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNotification({ type: "success", message: "Hak Akses berhasil dihapus!" });
    },
    onError: (err: any) => {
      setDeleteModalOpen(false);
      setNotification({ type: "error", message: err.response?.data?.error || "Gagal menghapus Hak Akses" });
    }
  });
    ...
    <div className="p-8 max-w-[1200px] mx-auto animate-in fade-in duration-500">
      <div className="mb-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-semibold tracking-tight text-[#0F172A]">Daftar Hak Akses (Role)</h1>
          </div>
          <p className="text-[14px] font-medium text-[#64748B]">Kelola level akses, izin sistem, dan kategori hak pengguna.</p>
        </div>
      ...
      <div className="enterprise-card flex flex-col bg-white border border-[#E2E8F0] overflow-hidden">
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                ...
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider w-48">Role ID</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider">Role Name</th>
                <th className="px-6 py-4 font-bold text-[#64748B] text-[11px] uppercase tracking-wider text-right">Status</th>
              </tr>
            </thead>
              ...
              {!isLoading && filteredRoles.map((r: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4.5 font-bold font-mono text-[12px] text-[#0F172A]">{r.id}</td>
                    <td className="px-6 py-4.5 font-bold text-[#0F172A]">{r.name}</td>
                    <td className="px-6 py-4.5 text-right font-bold text-[#94A3B8] text-[11px]">REGISTERED</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="p-5 border-t border-[#E2E8F0] flex flex-wrap items-center justify-between bg-[#F8FAFC]/30">
          ...
          <div className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-widest">
            {selectedRoleId ? `Role Terpilih: ${selectedRole?.name}` : 'Pilih role untuk melakukan modifikasi data'}
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-[#E2E8F0] w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-[#E2E8F0] flex items-center justify-between bg-white">
              <h2 className="text-xl font-bold text-[#0F172A]">
                {modalType === "create" ? "Tambah Hak Akses Baru" : modalType === "view" ? "Detail Hak Akses" : "Ubah Hak Akses"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[#64748B] uppercase tracking-widest">Role Name</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="pt-4 flex items-center gap-3">
                ...
                {modalType !== "view" && (
                  ...
                  >
                    ...
                    {modalType === "create" ? "Tambah Role" : "Simpan Perubahan"}
                  </button>
                )}
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setDeleteModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl border border-red-100 w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="p-8 text-center space-y-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-[#0F172A]">Delete Role?</h3>
                <p className="text-[14px] text-[#64748B] mt-1 px-4 leading-relaxed">
                  Are you sure you want to delete <span className="font-bold text-[#0F172A]">"{selectedRole.name}"</span>? Systems relying on this role might be affected.
                </p>
              </div>
